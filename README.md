...
Clicking the button sends a message from the popup to the content script running in the active tab. The content script runs three passes: a TreeWalker over all text nodes, a pass over anchor `href` attributes (to catch `mailto:` links whose display text isn't an email address), and a pass over input field values and placeholders. Each unique email is mapped to a generated fake address like `tohasi*@example.com` using a pronounceable random string. The asterisk makes fakes visually distinct from real addresses, and the mapping resets each time you click the button.

`example.com` is an internet domain permanently reserved for use in documentation and examples by [RFC 2606](https://www.rfc-editor.org/rfc/rfc2606), so fake addresses using it can never belong to a real person or organization.
...
Maskify scans the current page for email addresses and replaces every one with a realistic-looking fake address using the safe `example.com` domain. The same real address always maps to the same fake within a session, so the page stays consistent and readable without exposing anyone's personal information.
